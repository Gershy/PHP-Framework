...
abstract class AbstractInput extends AjaxListener {
	...
	*/
	...
	public function sessionClear() {
		App::$instance->clearSessionItem($this);
	}
...
class Form extends ContainerInput {
	...
	//Closure: $this->onSubmit = function($vals, $errs, $form) { ... };
	public $onSubmit = null;
	
	//Is set to reference the clicked submit button when the form is submitted
	...
	*/
	private function save($dat) {
		...
		if (!empty($submits)) {
			$this->submitButton = $submits[0];
			//Run the arbitrary onClick handler if it isn't null
			$func = $this->submitButton->onClick;
	...
	*/
	private function processSubmission($dat) {
		//TODO: submissions should fail if the related submit button is disabled
		$app = App::$instance;
		...
		if ($this->debug) {
			show($this->submitButton->name, 'Submit term');
...
class ListInput extends CompoundInput {
	...
	public function ListInput($key, $template) {
		if (DEBUG && strpos($template->getKey(), '{n}') === false)
			throw new Exception('ListInput template should have "{n}" as a substring of its key');
		$this->template = $template;
		
		/*
		Ensure the template is initialized before calling the parent constructor
		This is because $this->getValue() and $this->setValue() may be called from 
		the constructor, and the template is required for the ListInput 
		implementation of these functions.
		*/
		parent::CompoundInput($key, 'json');
	
		//Set a default value
		$dup = $this->template->duplicate(array('{n}' => 0));
		$this->setValue(array(
			$dup->getKey() => $dup->getValue()
		));
		
		$pass = $this;
		$this->addHandler = function($term) use($pass) { $pass->addItem(); 		App::$instance->refresh(); };
		$this->remHandler = function($term) use($pass) { $pass->remItem($term); App::$instance->refresh(); };
