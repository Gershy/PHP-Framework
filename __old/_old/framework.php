...
class App {
	...
	*/
	...
	public function setFormItem($formName, $itemName, $data) {
		if (!isset($this->session['_form'][$formName])) $this->session['_form'][$formName] = array();
		$this->session['_form'][$formName][$itemName] = $data;
	}
	
	public function getFormItem($formName, $itemName) {
		return isset($this->session['_form'][$formName][$itemName]) 
			? $this->session['_form'][$formName][$itemName]
			: null;
	}
	
	public function remFormData($name) {
		unset($this->session['_form'][$name]);
	}
