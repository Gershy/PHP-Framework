	...
	*/
	...
	public function sessionPush() {
		...
		if ($form instanceof Form) {
			App::$instance->setFormItem($form->getKey(), $this->getAbsoluteKey(), $this->getValue());
		}
	}
...
class Form extends ContainerInput {
	...
	*/
	...
	/*
	Forms clear old session data before pushing in the new data
	*/
	public function sessionPush() {
		App::$instance->setFormData($this->getKey(), array());
		parent::sessionPush();
	}
	...
	*/
	private function processInitialization($dat) {
		...
		// show($dat);
		// show($this->getInputTree()->map(function($inp) { return $inp->getValue(); })->getLeaves());
		// show($this->getInputTree()->map(function($inp) { return $inp->getValue(); })->getLeaves());
	...
	*/
	private function processSubmission($dat) {
		...
		// show($dat, 'DAT');
		// show($this->getValue(), 'getValue()');
		// show(App::$instance->getFormData($this->key), 'REL');
		// exit;
...
class ListInput extends CompoundInput {
	...
	public function ListInput($key, $template, $format = 'json') {
		...
		$pass = $this;
		$this->addHandler = function($term) use($pass) {
			$val = $pass->getValue();
			$dup = $pass->template->duplicate(array('{n}' => ''.count($val)));
			
			$val[$dup->getKey()] = $dup->getValue();
			$pass->setup($val);
			
			//Form looks
			show($pass->getRoot()->getValue());
			//exit;
			
			//TODO: this is inefficient!!!!! pushes the WHOLE form into the session, which is silly
			$pass->getRoot()->sessionPush();
		};
		$this->remHandler = function($term) use($pass) {
			show('Removed: '.$term); exit;
		};
	}
	
	public function initializeInput() {
		//Keep in mind $this->setup will call parent::initializeInput()
		$this->setup($this->getValue());
	}
	...
	public function setup($val) {
		...
		if ($this->length > 0) {
			$count = 0;
			foreach ($val as $v) {
				$inp = $this->addInput($this->template->duplicate(array('{n}' => "$count")));
				$inp->setValue($v);
				
				$rem = $this->addInput(new SubmitInput("remove-{$this->key}-$count"));
